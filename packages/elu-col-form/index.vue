  ...
  <el-form ref="form" :model="formData" :rules="formRules" :label-width="labelWidth" :label-position="labelPosition"
    ...
    <el-row type="flex" justify="end">
      ...
      <template v-if="showDefaultButtons">
        <el-button type="primary" :icon="submitIcon" :loading="submitLoading" @click="handleSubmit" size="small">
          {{ submitText }}
        </el-button>
        <el-button :icon="resetIcon" @click="handleReset" size="small">
          {{ resetText }}
        </el-button>
        <!-- 展开/收起按钮 -->
...
export default {
  ...
  computed: {
    ...
    formConfig() {
      if (this.isExpanded) {
        ...
        // 如果收起，只显示指定数量的字段
        return this.config.slice(0, this.visibleFieldCount);
      }
    },
    // 是否有更多字段可以展开
    hasMoreFields() {
      return this.config.length > this.visibleFieldCount;
    },
    // 表单验证规则
    formRules() {
      const rules = {};
  ...
  async mounted() {
    ...
    // 初始化显示字段数量
    this.visibleFieldCount = this.defaultVisibleFields || 6;
  },
  methods: {
    // 提交表单
    handleSubmit() {
